...
This python file is used to
    - Extract the tweet features of fake followers bots
"""
...
# WE FIRST GET THE FAKE FOLLOWERS' TWEETS FROM THE CSV FILE, EXTRACT THE NECESSARY TWEET ATTRIBUTES TO ANOTHER DATAFRAME,
# CHANGE TWEET TEXT VALUES TO STRINGS AND FINALLY SAVE THE FILES THAT HAVE THE ATTRIBUTES WE NEED
"""fakeFollowersTweets = pd.read_csv(str(os.getcwd()) + '/cresci-2017/Fake_Followers/tweets.csv', low_memory=False, encoding='latin-1')
fakeFollowersTweets = fakeFollowersTweets[
    ["id", "user_id", "text", "retweet_count", 'favorite_count', "created_at", "num_hashtags", "num_urls", "num_mentions"]]
fakeFollowersTweets = fakeFollowersTweets.astype({"text": 'string'})
fakeFollowersTweets.to_csv(str(os.getcwd()) + '/My Model/Fake Followers Tweets.csv', index=False)  # All tweets of social spam bots"""

fakeFollowersTweets = pd.read_csv(str(os.getcwd()) + '/My Model/Fake Followers Tweets.csv',
                                  low_memory=False)  # Get the tweets of the fake followers, reduced attribute version
fakeFollowersTweets = fakeFollowersTweets.astype({"text": 'string'})  # Change the tweet texts to string
fakeFollowersTweets['text'] = fakeFollowersTweets['text'].fillna(value='')  # Fill all empty values in the tweet texts with empty string
fakeFollowers = pd.read_csv(str(os.getcwd()) + '/cresci-2017/Fake_Followers/users.csv')  # Get the users that are fake followers
fakeFollowers['label'] = 1  # Label the bots
goalFakeFollowers = fakeFollowers[['label', 'id']]  # Create dataframe with id and labels
...
def extractTweetFeatures(traditionalFinalFeatures):
    ...
    sample = fakeFollowersTweets.loc[fakeFollowersTweets['user_id'] == traditionalFinalFeatures['id']]
...
goalFakeFollowers = goalFakeFollowers.apply(extractTweetFeatures, axis=1)  # Apply feature extraction to all labeled users
# goalFakeFollowers = goalFakeFollowers.drop(['id'], axis=1)
goalFakeFollowers.to_csv(str(os.getcwd()) + '/My Model/Fake Followers Tweet Features ID.csv', index=False)  # Save tweet features of the user type