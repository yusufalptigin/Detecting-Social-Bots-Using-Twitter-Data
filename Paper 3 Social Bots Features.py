...
This python file is used to
    - Extract the tweet features of social bots
"""
...
# WE FIRST GET THE SOCIAL BOTS' TWEETS FROM THE CSV FILE, EXTRACT THE NECESSARY TWEET ATTRIBUTES TO ANOTHER DATAFRAME,
# CHANGE TWEET TEXT VALUES TO STRINGS AND FINALLY SAVE THE FILES THAT HAVE THE ATTRIBUTES WE NEED
"""socialBotsTweets = pd.read_csv(str(os.getcwd()) + '/cresci-2017/Social_Spambots/tweets.csv', low_memory=False)
socialBotsTweets = socialBotsTweets[
    ["id", "user_id", "text", "retweet_count", 'favorite_count', "created_at", "num_hashtags", "num_urls", "num_mentions"]]
socialBotsTweets = socialBotsTweets.astype({"text": 'string'})
socialBotsTweets.to_csv(str(os.getcwd()) + '/My Model/Social Bots Tweets.csv', index=False) # All tweets of social spam bots"""

socialBotsTweets = pd.read_csv(str(os.getcwd()) + '/My Model/Social Bots Tweets.csv')  # Get the tweets of the social bots, reduced attribute version
socialBotsTweets = socialBotsTweets.astype({"text": 'string'})  # Change the tweet texts to string
socialBotsTweets['text'] = socialBotsTweets['text'].fillna(value='')  # Fill all empty values in the tweet texts with empty string
socialBots = pd.read_csv(str(os.getcwd()) + '/cresci-2017/Social_Spambots/users.csv')  # Get the users that are social bots
socialBots['label'] = 1  # Label the bots
goalSocialBots = socialBots[['label', 'id']]  # Create dataframe with id and labels
...
def extractTweetFeatures(traditionalFinalFeatures):
    ...
    sample = socialBotsTweets.loc[socialBotsTweets['user_id'] == traditionalFinalFeatures['id']]
...
goalSocialBots = goalSocialBots.apply(extractTweetFeatures, axis=1)  # Apply feature extraction to all labeled users
# goalSocialBots = goalSocialBots.drop(['id'], axis=1)
goalSocialBots.to_csv(str(os.getcwd()) + '/My Model/Social Bots Tweet Features ID.csv', index=False)  # Save tweet features of the user type
